...
function KNButton:new(pathStr,t,x,y,backFun,id,params)
	...
	function createItem(pathStr,t,x,y,id)
		...
		if t==IMAGEBUTTON then--图片按钮
			local buttonPath =  "buttonUI/"..pathStr.."/"
			-- print(buttonPath)
			-- if  io.exists(buttonPath.."def.png") then

   --          -- elseif string.find(buttonPath , "image") == 1 then
			...
			if type(this.params) == "table" and this.params["noDisable"] then
				if this.params["noPre"] then
					menuItem = CCMenuItemImage:create(buttonPath.."def.png",buttonPath.."def.png")
				else
