...
----------------------------------------------------------------
local client_item = class("client_item")
----------------------------------------------------------------
function client_item:ctor(data)
    self.__data = nil
	self:set_data(data)
end
----------------------------------------
function client_item:get_data()
    return self.__data
end
----------------------------------------
function client_item:set_data( data )
    self.__data = self:gen_new(data)
    -- dump(self.__data)
end
----------------------------------------
function client_item:get( key )
    if key == nil then
        return self.__data
    end
...
----------------------------------------
function client_item:setkey( key, data )
    self.__data[key] = data
end
----------------------------------------
-- 转为新数据，来自己服务端
-- 因为pb会附带其他没用信息，所有需要这步
function client_item:gen_new(olddata)
    local newdata = {
        GUID    = olddata.GUID,
