...
------------------------------------------------------------------------------
--[[ 遍历
	list:traversal(function(value)
	  --todo
	end)
]]
function CList:traversal( callback_ )
    local iter_where = self:head()
    local iter_end =  self:tail()
    while( iter_where ~= iter_end) do
        local value = iter_where:getvalue()
        if value  then callback_(value) end
        if( iter_where:inc() == false ) then break end
    end
end
------------------------------------------------------------------------------
return CList
