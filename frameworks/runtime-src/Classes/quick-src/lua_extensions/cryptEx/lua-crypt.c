...
lb64decode(lua_State *L) {
	...
	for (i=0;i<sz;) {
		...
		case 1:
			if (c[3] != -2 || (c[2] & 3)!=0) {
				return luaL_error(L, "Invalid base64 text case 1");
			}
		...
		case 2:
			if (c[3] != -2 || c[2] != -2 || (c[1] & 0xf) !=0)  {
				return luaL_error(L, "Invalid base64 text case 2");
			}
			...
			break;
		default:
			return luaL_error(L, "Invalid base64 text default");
		}
	}
