...
_pack_packed_varint(struct _pattern_field *pf , struct pbc_slice *slice, pbc_array array) {
	...
	n = pbc_array_size(array);
	estimate = n; 
	estimate_len = _pack_wiretype(estimate , &s);
	if (estimate_len < 0) {
		return -1;
	}
	
	for (i=0;i<n;i++) {
		len	= _pack_number(pf->ptype, CTYPE_VAR , &s, _pbcA_index_p(array, i));
		if (len < 0)
			return -1;
		packed_len += len;
	}
	...
	header_len = _pack_wiretype(packed_len , &header_slice);
...
pbc_pattern_pack(struct pbc_pattern *pat, void *input, struct pbc_slice * s)
{
	...
	len = (char *)slice.buffer - (char *)s->buffer;
	ret = s->len - len;
	s->len = len;
	return ret;
}
...
pbc_pattern_unpack(struct pbc_pattern *pat, struct pbc_slice *s, void * output) {
	...
	r = _pbcC_open(_ctx, s->buffer, s->len);
	if (r <= 0) {
	...
	ctx = (struct context *)_ctx;
	field = (bool *)alloca(pat->count * sizeof(bool));
	memset(field, 0, pat->count * sizeof(bool));

	for (i=0;i<ctx->number;i++) {
		f = bsearch_pattern(pat, ctx->a[i].wire_id >> 3);
		if (f) {
			index = f - pat->f;
...
pbc_pattern_new(struct pbc_env * env , const char * message, const char * format, ... ) {
	...
	for (i=0;i<n;i++) {
		f = &(pat->f[i]);
		field = (struct _field *)_pbcM_sp_query(m->name, ptr);
		if (field == NULL) {
