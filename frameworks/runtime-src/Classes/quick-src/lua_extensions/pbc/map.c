...
_pbcM_si_new(struct map_kv * table, int size)
{
	int empty = 0;
	int i;
	struct _pbcM_si_slot * empty_slot;
...
_pbcM_ip_new_hash(struct map_kv * table, int size)
{
	...
	int hash;
	struct _pbcM_ip_slot * slot;
	struct _pbcM_ip_slot * empty_slot;
...
_pbcM_ip_new(struct map_kv * table, int size)
{
	...
	ret = (struct map_ip *)malloc(sizeof(struct map_ip));
	ret->hash_size = size;
	ret->slot = NULL;
...
_pbcM_ip_query(struct map_ip * map, int id)
{
	...
	hash = (unsigned)id % map->hash_size;
	slot = &map->slot[hash];
	for (;;) {
		if (slot->id == id) {
			return slot->pointer;
...
{
	struct _pbcM_sp_slot * empty_slot;
