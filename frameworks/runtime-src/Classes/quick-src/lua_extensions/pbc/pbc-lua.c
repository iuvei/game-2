...
_rmessage_new(lua_State *L) {
	...
	m = pbc_rmessage_new(env, type_name, &slice);
	if (m==NULL)
		return 0;
...
_rmessage_int52(lua_State *L) {
	...
	v = (int64_t)((uint64_t)hi << 32 | (uint64_t)low);
...
_env_type(lua_State *L) {
	...
	lua_settop(L,3);
	env = (struct pbc_env *)checkuserdata(L,1);
	...
	type = NULL;
	ret = pbc_type(env, type_name, key, &type);
...
_pattern_unpack(lua_State *L) {
	...
	format = lua_tolstring(L,2,&format_sz);
	...
	ret = pbc_pattern_unpack(pat, &slice, temp);
	if (ret < 0) {
		return 0;
	}
	...
	for (i=0;i<format_sz;i++) {
		...
		if (type >= 'a' && type <='z') {
			...
			array = true;
			n = pbc_array_size((struct _pbc_array *)ptr);
...
//	pbc_pattern_set_default(pat, data);
	...
	for (;;) {
		...
		ret = pbc_pattern_pack(pat, data, &slice);
...
_decode(lua_State *L) {
	...
	n = pbc_decode(env, type, &slice, decode_cb, L);
	if (n<0) {
		lua_pushboolean(L,0);
	} else {
		lua_pushboolean(L,1);
...
_add_pattern(lua_State *L) {
	...
	pat = (struct pbc_pattern *)lua_touserdata(L,2);
...
_add_rmessage(lua_State *L) {
	...
	msg = (struct pbc_rmessage *)lua_touserdata(L,2);
