...
pbc_decode(struct pbc_env * env, const char * type_name , struct pbc_slice * slice, pbc_decoder pd, void *ud) {
	...
	count = _pbcC_open(_ctx,slice->buffer,slice->len);
	if (count <= 0) {
	...
	start = (uint8_t *)slice->buffer;

	
	for (i=0;i<ctx->number;i++) {
		int id = ctx->a[i].wire_id >> 3;
		f = (struct _field *)_pbcM_ip_query(msg->id , id);
		if (f==NULL) {
			...
			a = &ctx->a[i];
			n = call_array(pd, ud, f , start + a->v.s.start , a->v.s.end - a->v.s.start);
			if (n < 0) {
				_pbcC_close(_ctx);
				return -i-1;
			}
