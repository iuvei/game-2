...
_concat_name(struct _stringpool *p , const char *prefix ,  int prefix_sz , const char *name , int name_sz, int *sz) {
	...
	temp = (char *)alloca(name_sz + prefix_sz + 2);
	...
	ret = _pbcS_build(p , temp, name_sz + prefix_sz + 1);
	return ret;
}
...
_register_enum(struct pbc_env *p, struct _stringpool *pool, struct pbc_rmessage * enum_type, const char *prefix, int prefix_sz) {
	...
	name = pbc_rmessage_string(enum_type, "name", 0 , &name_sz);
	temp = _concat_name(pool, prefix , prefix_sz , name , name_sz, NULL);
...
_register_field(struct pbc_rmessage * field, struct _field * f, struct _stringpool *pool) {
	...
	default_value = pbc_rmessage_string(field, "default_value", 0 , &vsz);
...
_register_extension(struct pbc_env *p, struct _stringpool *pool , const char * prefix, int prefix_sz, struct pbc_rmessage * msg, pbc_array queue) {
	...
	for (i=0;i<extension_count;i++) {
		...
		extendee = pbc_rmessage_string(extension , "extendee" , 0, NULL);
...
_register_message(struct pbc_env *p, struct _stringpool *pool, struct pbc_rmessage * message_type, const char *prefix, int prefix_sz, pbc_array queue) {
	...
	enum_count = pbc_rmessage_size(message_type, "enum_type");
	...
	// nested type
	message_count = pbc_rmessage_size(message_type, "nested_type");
...
_register(struct pbc_env *p, struct pbc_rmessage * file, struct _stringpool *pool) {
	...
	int i;
	int message_count;
	struct pbc_rmessage * enum_type = NULL;
	struct pbc_rmessage * message_type = NULL;
	...
	enum_count = pbc_rmessage_size(file, "enum_type");
	

	for (i=0;i<enum_count;i++) {
		enum_type = pbc_rmessage_message(file, "enum_type", i);
		_register_enum(p,  pool , enum_type, package, package_sz);
	}

	message_count = pbc_rmessage_size(file, "message_type");
	for (i=0;i<message_count;i++) {
		message_type = pbc_rmessage_message(file, "message_type", i);
...
//	printf("reg :%s\n",filename);
	...
	for (i=0;i<sz;i++) {
		dname = pbc_rmessage_string(file,"dependency",i,NULL);
//		printf("dependency :%s\n",dname);
...
_register_no_dependency(struct pbc_env * p,struct pbc_rmessage ** files , int n ) {
	...
	for (i=0;i<n;i++) {
		...
		err = _check_file_name(p, files[i], &filename);
		switch(err) {
...
pbc_register(struct pbc_env * p, struct pbc_slice *slice) {
	...
	n = pbc_rmessage_size(message, "file");
	files = (struct pbc_rmessage **)alloca(n * sizeof(struct pbc_rmessage *));
	...
	r = n;
	do {
		rr = _register_no_dependency(p,files , n);
		if (rr == r) {
